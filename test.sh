#To use single case testing, write name of test as first argument like this: ./test.sh example.txt
RED='\033[0;031m'
GREEN='\033[0;32m'
X='\033[47m'
OFF='\033[0m'
...
if [ -z "$1" ]; then
...
else    #Testing just one case
    echo "testujem: $TEST"
    for i in "${tests_syntax[@]}"
    do
        if [ "$i" = "$TEST" ] ; then
            echo "Test: $TEST"
            ./Main < ./tests/syntax/$TEST 2> ./tests/cache.txt
            if [[ $(< ./tests/cache.txt) != "syntax error" ]]; then
                echo -e "${RED}TEST FAILED!${OFF}"
                echo $(< ./tests/cache.txt)
            else
    ...
    do
        if [ "$i" = "$TEST" ] ; then
            echo "Test: $TEST"
            ./Main < ./tests/semantic/$TEST 2> ./tests/cache.txt
            if ! grep -q "^semantic error:" ./tests/cache.txt; then
                echo -e "${RED}TEST FAILED!${OFF}"
                echo $(< ./tests/cache.txt)
            else
    ...
    do
        if [ "$i" = "$TEST" ] ; then
            echo "Test: $TEST"
            ./Main < ./tests/math/$1 > ./tests/cache.txt
            if [[ $(< ./tests/cache.txt) == "OK" ]]; then
                echo -e "${GREEN}TEST PASSED!${OFF}"
                echo $(< ./tests/cache.txt)
            else
    ...
    echo "Given test does not exist: $TEST"
fi